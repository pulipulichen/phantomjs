...
config.cache_dir = fs.workingDirectory + "/cache/";
config.output = config.cache_dir + "/output.html";
config.url;
...
var _open_webpage = function (config, _callback) {
	...
	var onPageReady = function() {
		if (config.selector !== undefined) {
			var htmlContent = eval('webpage.evaluate(function () {'
				+ '	htmlContent = $("<div></div>").append($(document).find("' + config.selector + '").clone()).html();'
				+ '	return htmlContent;'
			+ '})');
		}
		else {
	...
	webpage.open(config.url, function() {
		webpage.injectJs(wd + '/lib/jquery.js');
